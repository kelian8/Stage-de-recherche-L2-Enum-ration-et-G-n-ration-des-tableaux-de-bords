...
std::vector<int> BorderToPlist(std::vector<int> b){
  std::vector<int> L; //liste vide pour renvoyer qui sera resultat
...
std::vector<int> PrefixToList(const std::vector<int>& P) {
  std::vector<int> L;  //creation d'une liste vide pour les resultats
  int i = P.size() - 1; //indice permettant de parcourir la liste de droite a gauche 
    ...
    while (i > 0) {
	  ...
	  if (j + P[j] - 1 >= i) {  //la condition
        ...
        int l;
        if (I.empty()) { // si la liste est vide l = 0 et on decremente i de 1
            l = 0;
            i = i - 1;
        } else {        //si elle n'est pas vide l = i - minI + 1 et on decremente i de minI-1
            int minI = *std::min_element(I.begin(), I.end());
            l = i - minI + 1;
            i = minI - 1;
        }
        
        L.insert(L.begin(), l);  // on ajoute l au debut de L
    }
...
std::vector<std::string> creaAlph(int n){//construit un alphabet [µ1,µ2,...µn-1]
